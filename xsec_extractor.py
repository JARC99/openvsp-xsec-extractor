...
if NORMAL_AXIS == 'x':
    ...
    xsecs_list = []
    for i in range(len(locs)):
        xsec = np.stack((xsecs_data[xsecs_data[:, 1] == locs[i]][:,0], xsecs_data[xsecs_data[:, 1] == locs[i]][:,-1]), 1)
        xsecs_list.append(xsec)  
    
for i, xsec in enumerate(xsecs_list):
    h_center_val = (np.max(xsec[:, 0]) - np.min(xsec[:, 0]))/2 + np.min(xsec[:, 0])        
    v_center_val = (np.max(xsec[:, 1]) - np.min(xsec[:, 1]))/2 + np.min(xsec[:, 1]) 
    ...
    sorted_quad1_points = quad1_points[quad1_points[:, 0].argsort()[::-1]]
    sorted_quad4_points = quad4_points[quad4_points[:, 0].argsort()]
    sorted_righthalf_points = np.vstack((sorted_quad4_points, sorted_quad1_points))
    ...
    sorted_quad2_points = quad2_points[quad2_points[:, 0].argsort()[::-1]]
    sorted_quad3_points = quad3_points[quad3_points[:, 0].argsort()]
    sorted_lefthalf_points = np.vstack((sorted_quad2_points, sorted_quad3_points))
    
    sorted_xsec = np.vstack((sorted_righthalf_points, sorted_lefthalf_points))
    
    norm_xsec = np.stack((sorted_xsec[:, 0] - h_center_val, sorted_xsec[:, 1] - v_center_val), 1)
    
    fig = plt.figure(dpi=DPI)
    ax = fig.add_subplot(111)
    if NORMAL_AXIS == 'x':
        ax.set_title("XSec_{0}, x = {1} m".format(i, round(locs[i], 3)))
        ax.axis('equal')
        ax.set_xlabel("y, m")
        ax.set_ylabel("z, m")
    else:
        ax.set_title("XSec {0}, y = {1} m".format(i, round(locs[i], 3)))
        ax.axis('equal')
        ax.set_xlabel("x, m")
        ax.set_ylabel("z, m")
